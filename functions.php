...
function saintsmedia_scripts()
{
	...
	wp_enqueue_script('admin-bar-check', get_template_directory_uri() . '/js/admin-bar-check.js', array(), _S_VERSION, true);
 ...
 * Automatically selects menu based on URL language prefix
 *
 * URL patterns:
 * - example.com/page → 'en' (default, no prefix)
 * - example.com/es/page → 'es'
 * - example.com/de/page → 'de'
 * - example.com/fr/page → 'fr'
 * - example.com/ru/page → 'ru'
 *
 * Admin setup: WordPress > Appearance > Menus
 * 1. Create menus: "Menu EN", "Menu ES", "Menu DE", etc.
 * 2. Go to Menu Settings (bottom of menu page)
 * 3. Check which locations each menu should use:
 *    - "Menu EN" → assign to "Primary Menu (English)"
 *    - "Menu ES" → assign to "Primary Menu (Spanish)"
 *    - etc.
 * 4. Click "Save Menu"
 *
 * System automatically detects language from URL and displays correct menu!
 */

/**
 * Get available languages from Customizer settings
 *
